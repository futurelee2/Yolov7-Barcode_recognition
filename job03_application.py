                ...
                for d in decoded:
                    ...
                    try:
                        ...
                        try:
                            ...
                            TTS_text = '이 제품은 {}이고, {}카테고리의 제품입니다.'.format(TTS_data[0], TTS_data[2]) # TTS로 재생할 부분
                            TTS_text_label = '이 제품은 {}이고, \n {}카테고리의 제품입니다.'.format(TTS_data[0], TTS_data[2]) # APP 내에 출력할 부분
                            # 멀티프로세싱 코드 (target = 해당 동작 , args = 입력값)
                            threading.Thread(target=self.mainWindow.say, args=(TTS_text,)).start()
                            self.mainWindow.lbl_result.setText(TTS_text_label)
...
class App_Barcode(QWidget, form_window):
    ...
    def say(self, TTS_text):                    # TTS 재생 함수
        engine = pyttsx3.init()
        engine.setProperty('rate', 150)         # 기본 속도 200
