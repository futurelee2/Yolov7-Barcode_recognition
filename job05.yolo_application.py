...
class PThread(QThread):
    ...
    def run(self, opt):  # 쓰레드로 동작시킬 함수 내용 구현(런에 원하는 동작을 넣어줘야한다!)
        while True:
            self.detect(save_img=False)
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(
                                -1).tolist()  # normalized xywh
                            print(xywh[:])
                            dist = PThread.Distance_Measurement(width, height)
                        ...
                        if len(overlist) != 0:
                            ...
                            threading.Thread(target=Exam.say, args=(TTS_text,)).start()
