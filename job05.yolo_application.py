...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, clean_str
...
class Exam(QWidget, form_window):
    ...
    def __init__(self):  # mainWindow는 QWidget에서 전달하는 self이다.(QWidget의 인스턴스)
        super().__init__()
        self.setupUi(self)
        ...
        with torch.no_grad():
            if self.opt.update:  # update all models (to fix SourceChangeWarning)
                for self.opt.weights in ['yolov7-tiny.pt']:
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(
                                -1).tolist()  # normalized xywh
                            print(xywh[:])
                            dist = self.Distance_Measurement(width, height)
                        ...
                        if len(overlist) != 0:
                            overlist_str = ' '.join(overlist)
                            TTS_text = 'There is a danger around.'
                            print(TTS_text)
                            # 멀티프로세싱 코드 (target = 해당 동작 , args = 입력값)
                            threading.Thread(target=self.say, args=(TTS_text,)).start()
